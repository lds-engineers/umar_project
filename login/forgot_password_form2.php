...
class login_forgot_password_form extends moodleform {
    ...
    function definition() {
        ...
        $mform    = $this->_form;
        $mform->addElement('html','<div class="skills-forgot-password flex-column">');
        $mform->setDisableShortforms(true);

        // Hook for plugins to extend form definition.
        core_login_extend_forgot_password_form($mform);
       
        $mform->addElement('html','<div class="text-left"><h3> Forgot your password?</h3><p>Enter your email address to reset your password</p>');
       ...
       /* $mform->addElement('header', 'searchbyemail', get_string('searchbyemail'), '');*/
        ...
        $mform->addElement('html','<div class="sss">');
        $mform->addElement('html','<div><i class="fa fa-envelope" aria-hidden="true"></i>
</div>');

        $mform->addElement('text', 'email','', 'maxlength="100" size="30"' . $purpose);
        $mform->setType('email', PARAM_RAW_TRIMMED);
        $mform->addElement('html','</div>');

        $submitlabel ="Reset Password";
        $mform->addElement('submit', 'submitbuttonemail', $submitlabel);
        $mform->addElement('html','<div class="text-right login_link  "><p><a href="'.$CFG->wwwroot.'/login">Login</a></p></div>');
        $mform->addElement('html','</div>');
    }
